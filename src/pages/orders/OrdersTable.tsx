          ...
          <tbody className="divide-y divide-slate-100/40 bg-white">
            {
              orders.map((order) => (
                <tr key={order.id} className="hover:bg-slate-100 transition-colors">
                  ...
                  <td className="px-6 py-3 text-sm text-slate-700">{new Date(order.estimated_delivery_date).toLocaleDateString()}</td>
                  <td className="px-6 py-3 text-sm text-slate-700">₡{order.amount_charged.toFixed(2)}</td>
                </tr>
              ))
            }
          </tbody>
